...
class WeightedSmoothL1Loss(nn.Module):
    ...
    def forward(self, input: torch.Tensor, target: torch.Tensor, weights: torch.Tensor = None):
        """
        Args:
            input: (B*64, #anchors, #codes) float tensor. Ecoded predicted locations of objects.
            target: (B*64, #anchors, #codes) float tensor. Regression targets.
            weights: (#anchors) float tensor if not None.
        Returns:
            loss: (B*64, #anchors) float tensor.
