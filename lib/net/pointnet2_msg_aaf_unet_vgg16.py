...
#         super(IA_Layer, self).__init__()
#         self.ic, self.pc = channels
#         self.conv1=nn.Sequential(nn.Conv1d(self.ic+self.pc,self.pc,1),
#                                  nn.ReLU())
#         self.conv2 = nn.Conv1d(self.pc, 1, 1)
#         self.conv3 = nn.Sequential(nn.Conv1d(self.ic, self.pc, 1),
#                                    nn.BatchNorm1d(self.pc),
#                                    nn.ReLU())
#
#     def forward(self, img_feas, point_feas):
#         fusion_feas = torch.cat([point_feas,img_feas],dim=1) #[B,C,N]
#         fusion_feas = self.conv1(fusion_feas)
#         att = F.sigmoid(self.conv2(fusion_feas)) #[B,1,N]
#         img_feas_new = self.conv3(img_feas)
#         out = img_feas_new * att
#
#         return out
...
class IA_Layer(nn.Module):
    def __init__(self, channels):
        ...
        self.ic, self.pc = channels
        self.rc = self.pc//4
        self.conv1=nn.Conv1d(self.ic,self.rc,1)
        self.conv2=nn.Conv1d(self.pc,self.rc,1)
        self.conv3 = nn.Conv1d(self.rc, 1, 1)
        self.conv4 = nn.Sequential(nn.Conv1d(self.ic, self.pc, 1),
                                   nn.BatchNorm1d(self.pc),
                                   nn.ReLU())

    def forward(self, img_feas, point_feas):
        new_img_feas=self.conv1(img_feas)
        new_point_feas = self.conv2(point_feas)
        att = F.sigmoid(self.conv3(F.relu(new_img_feas+new_point_feas))) #[B,1,N]
        img_feas_new = self.conv4(img_feas)
        out = img_feas_new * att
