...
#partA2_rcnn_reg_loss
def get_box_reg_layer_loss(ret_dict):
    loss_cfgs = cfg.RCNN.LOSS_CONFIG
    roihead=RoIHeadTemplate(num_class=1,model_cfg=cfg.RCNN)
    code_size = roihead.box_coder.code_size #7
    reg_valid_mask = ret_dict['reg_valid_mask'] #[B*64]-iou_score>0.55的01
    gt_boxes3d_ct = ret_dict['gt_of_rois'] #[B*64,7]
    gt_of_rois_src = ret_dict['gt_of_rois_src'] #[B*64,7]
    rcnn_reg = ret_dict['rcnn_reg'] #[B*64,7]
    roi_boxes3d = ret_dict['roi_boxes3d'] #[B*64,7]

    fg_mask = (reg_valid_mask > 0)
    fg_sum = fg_mask.long().sum().item()

    if loss_cfgs.REG_LOSS == 'smooth-l1':
        rois_anchor = roi_boxes3d.clone().detach()
        rois_anchor[:, 0:3] = 0#xyz
        rois_anchor[:, 6] = 0 #ry

        reg_targets = roihead.box_coder.encode_torch(gt_boxes3d_ct, rois_anchor)#[B*64,7]

        from lib.pcdet.utils.loss_utils import WeightedSmoothL1Loss
        rcnn_loss_reg = WeightedSmoothL1Loss(code_weights=cfg.RCNN.LOSS_CONFIG.LOSS_WEIGHTS.get('code_weights', None))(
            rcnn_reg.unsqueeze(dim=0),
            reg_targets.unsqueeze(dim=0)
        )# [B, 64, 7]
        rcnn_loss_reg = (rcnn_loss_reg.view(-1, code_size) * fg_mask.unsqueeze(dim=-1).float()).sum() / max(fg_sum, 1)
        ...
        if loss_cfgs.CORNER_LOSS_REGULARIZATION and fg_sum > 0:
            ...
            #loss_corner
            from lib.pcdet.utils import loss_utils
